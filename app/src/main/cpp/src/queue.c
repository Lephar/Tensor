...
VkCommandBuffer beginSingleComputeCommand() {
    VkCommandBuffer commandBuffer = allocateSingleCommandBuffer(&computeQueue);
...
void endSingleCommand(Queue *queue, VkCommandBuffer *commandBuffer) {
    ...
    vkQueueSubmit(queue->queue, 1, &submitInfo, 0);
...
void endSingleComputeCommand(VkCommandBuffer commandBuffer) {
    endSingleCommand(&computeQueue, &commandBuffer);
}
